...
if(row > cur_game->rows || col > cur_game->cols || row < 0 || col < 0) return NULL;
    ...
    for(i =0; i<rows; i++){
      ...
      for(k = 0; k <= i; k++){
         ...
         if(cur_game->cells[k*cols + j] == -1){
           ...
           if(shift || k == i){
    if(k != 0 && cur_game->cells[k*cols+j] == cur_game->cells[(k-1)*cols+j] && combine[k-1][j] == 0){
           check = 1;
             ...
             if(cur_game->cells[k*cols + j] == -1){
              ...
              cur_game->cells[k*cols+j] = cur_game->cells[i*cols +j];
               cur_game->cells[i*cols+j] = -1;
                 ...
                 if(shift || k == i){
        if(k != (rows-1) && cur_game->cells[k*cols+j] == cur_game->cells[(k+1)*cols+j] && combine[k+1][j] == 0){
                   check = 1;
           cur_game->cells[(k+1)*cols + j] = cur_game->cells[k*cols + j] * 2;
                 cur_game->score  += cur_game->cells[k*cols + j]*2;
                  cur_game->cells[k*cols + j] = -1;
     ...
     for(j = 0; j < cols; j++){
      if(cur_game->cells[i*cols+j] != -1){
       ...
       cur_game->cells[i*cols+k] = cur_game->cells[i*cols +j];
         ...
         if(shift || k == j){
     if(k != 0 && cur_game->cells[i*cols+k] == cur_game->cells[i*cols+k-1] && combine[i][k-1] == 0){
              check = 1;
...
int move_d(game * cur_game){ //slide to the right
    ...
    for(i =0; i<rows; i++){
     ...
     cur_game->cells[i*cols+k] = cur_game->cells[i*cols +j];
	   ...
	   if(shift || k == j){
         if(k != cols-1 && cur_game->cells[i*cols+k] == cur_game->cells[i*cols+k+1] && combine[i][k+1] == 0){
           check = 1;
        cur_game->cells[(i*cols) + k+1] = cur_game->cells[i*cols +k] * 2;
           cur_game->score  += cur_game->cells[i * cols + k]*2;
           cur_game->cells[i*cols + k] = -1;
 ...
 */
{
    ...
    for (i=0; i<row; i++){
        for (j=0; j<col; j++){
            if (cur_game->cells[i*col+j] == -1) x = 1;
